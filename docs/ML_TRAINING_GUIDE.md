...
Зараз dataset також фіксує параметри demand-aware policy: штраф для порожніх
підходів, бонус для забитих смуг, таймер очікування нового попиту та
використання штатних SUMO-тривалостей фаз. Старі `current`-моделі залишаються
сумісними, бо runtime читає `feature_columns` з metadata artifact і передає
моделі саме ті колонки, з якими її тренували.

Runtime-дані збираються не з усієї карти, а з локальних sensor windows біля
контрольованих перехресть. `sensor_range_meters` задає, скільки метрів
вхідної смуги перед stop-line та вихідної смуги після перехрестя бачить
умовна камера або датчик. Цей параметр входить у dataset і training features,
щоб модель відповідала реалістичному способу збору даних.
