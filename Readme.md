...
Дані для керування не читаються “по всій карті”. У runtime FlowMind працює
як система локальних датчиків біля контрольованих перехресть: для кожного
вхідного та вихідного напрямку береться тільки зона в межах заданої дальності
датчика, наприклад 120 метрів. У реальному місті це відповідає камері,
радару, індукційній петлі або іншому датчику, який рахує, скільки авто
під’їжджає до перехрестя, скільки стоїть у черзі та чи є місце після
перехрестя.
...
* скоротити або не продовжувати зелений, якщо напрямок порожній;
* дати пріоритет напрямку з реальною чергою або високою зайнятістю смуги;
* запускати таймер очікування для напрямку, де щойно з’явилась машина;
* використовувати штатні тривалості фаз SUMO як базову поведінку конкретного
  перехрестя;
...
Пішоходів також можна враховувати, якщо SUMO-сценарій містить pedestrian
flows і контрольовані crossing lanes. У такому разі вони мають бути окремим
шаром попиту з власною вагою очікування, а `SafetyValidator` має залишатися
фінальним обмежувачем, щоб не створювати небезпечні конфліктні фази.

У Project Manager є кнопка **Запустити повну систему**. Вона готує високий
сфокусований потік, запускає fixed-baseline, FlowMind із ML-прогнозом,
швидкою, green corridor, SUMO GUI та live dashboard.
